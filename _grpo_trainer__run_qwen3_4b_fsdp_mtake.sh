...
if command -v nvidia-smi >/dev/null 2>&1; then
    NGPUS_PER_NODE=$(nvidia-smi --list-gpus | wc -l)
else
    NGPUS_PER_NODE=0
fi
echo "XXX NGPUS_PER_NODE: ${NGPUS_PER_NODE}" | tee -a ${LOGFILE}

# @@@ahoaho XXX
#if (( NGPUS_PER_NODE == 0 )); then
#    echo "ERROR: A GPU is required to run this command. Exiting..." | tee -a ${LOGFILE}
#    exit 1
#fi
if (( NGPUS_PER_NODE < 8 )); then
...
#ENV="MODEL_PATH=ibm-granite/granite-4.1-3b ${ENV}"  # default: Qwen/Qwen3-4B, OK with 1 node 8 gpus
...
# @@@ahoaho XXX
#cmd="${ENV}bash examples/grpo_trainer/run_qwen3_4b_fsdp.sh"
cmd="${ENV}bash examples_mtake/grpo_trainer/run_qwen3_4b_fsdp_mtake.sh"
...
#EXPERIMENT_NAME=glm41v_9b_function_rm
